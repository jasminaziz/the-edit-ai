...
Build Your Own Stack ruling.** Nothing was committed to `src/` **by the Cowork session**. **A code session then
took nine commits, `75792c1f` through `b4c5a7cb`, and did all nine restructure
items, the Stack cut and F2c.** It also found that `StatusBadge.tsx` and
`STATUS_MAP` were deleted on 2026-07-03 and had been carried as live dead code
ever since, and Jasmin found that `tsc --noEmit` is not the build, so **the
pre-commit gate is now three commands including `bun run build`.** **The card was then verified in the browser by Jasmin on 26 Aug and those checks
are closed.** What remains before launch is not code: it is a positioning pass
over every surface, opened as a separate thread on 26 Aug. See section 8.
...
| `reports/2026-08-26-copy-pack-card-restructure.md` | Approved 26 Aug. The card's zone label, the DPIA definition line, and the C4(b) Red-only placement change. Live until placed. |
| `reports/2026-08-26-toolcard-restructure.md` | The ToolCard restructure proposal, the verdict sector rule and the DPIA literacy gap. |
...
## 8. The pre-launch positioning pass, opened 26 August 2026

The build is materially done. What is not done is checking that every surface
says the same thing, and Jasmin's judgement is that it does not yet.

**Her framing, recorded verbatim enough to be usable.** The Edit is pivoting to
speak more clearly to charity and cultural organisations using AI, with the
governance lens highlighted alongside adoption. **But the main purpose has
always been to present an easy way into understanding the tools out there: what
they can be used for, in a fun and low-stress way.** The pivot sharpens who that
is for and adds the governance weight; it does not replace the original purpose.

**Open concerns, hers:**

- The tool cards may still be overloaded and `/tools` too busy, particularly on
  a phone and for a visitor arriving intimidated.
- **The card should say four things: what it is, how much it is, what it is
  used for, and why care should be taken when using it.** That fourth phrasing
  is softer and more human than the DPIA framing currently on the card, and the
  register difference is the point.
- The trustee template is liked, but **what a DPIA is, and why an organisation
  should take it seriously, needs more explanation than it currently gets.**
- The site must stay fun and personality-driven, in Jasmin's first-person voice,
  not too formal, with some lightness kept.
- **The other pages have not been brought into this vision at all.** `/`,
