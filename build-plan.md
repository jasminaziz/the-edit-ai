...
| Merge | Fast-forward `main` and `overhaul/sector-axis` to the branch, then verify on production | Jasmin's sign-off | Live on theeditai.co.uk and verified from a cold load | **done 13 Sep**: signed off by Jasmin; `main` and `overhaul/sector-axis` fast-forwarded from 459757d to 9b3793f, 32 commits; PR #2 marked merged by GitHub; production deploy READY; live bundle carries the new strings; production rendered cold at 1280 and 390 (nav, homepage sector line at 704 inside the fold, hero 500 on a phone, banner and checks line on /learning at 1280, no checks line at 390) |
...
**Ruled later on 13 September, through the question tool:**
- **The hero keeps no promise about the full stop.** The first board's cap put
  the pills over the full stop at 1280 and 1440 (61% and 70% measured), and no
  sizing clears it reliably, because the pile settles differently from run to
  run; today's hero already covers it partly at 1024. Jasmin chose the tuned
  cap from a second board, accepting some cover, over an invisible physics
  disc that would have guaranteed it.
- **The `/ai-news` source badge keeps its new position**, 20px lower and level
  with the subheading since the checks line made its text block taller.
- **The header body text rises from 60% to 85% cream**, so it passes AA (3.86:1
  to 6.24:1) and matches the checks line beneath it. Its own commit, outside
  the site map steps.
